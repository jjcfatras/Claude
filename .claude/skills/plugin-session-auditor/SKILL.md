...
- User says one of the plugins (`cherry-pick`, `merge`, `git`, `code-review`, `code-review-AT`, `respond-to-review`, `test-driven-fix`, `doc-audit`, `debate`, `simplify`, `transcript`, `jira-ticket`, `jira-implement`) misbehaved in a recent session.
