...
A Claude Code [plugin marketplace](https://docs.claude.com/en/docs/claude-code/plugin-marketplaces) shipping two slash commands and sixteen skills the author uses for everyday Git, code-review, documentation, and reasoning workflows.
...
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `git`               | `/git:commit` · `/git:commit-push` · `/git:commit-push-pr` · `/git:push` · `/git:clean_gone` · `/git:cherry-pick` · `/git:merge` | Git workflow: auto-message commit, commit + push (refreshes an open PR), commit + push + open PR, push already-committed work (also refreshes an open PR), prune local `[gone]` branches, plus cherry-pick and merge with conflict resolution. All seven ship as skills, so Claude can auto-invoke them in addition to the `/` commands.                                                                                                     |
...
| `code-review`       | `/code-review <pr-number> [--auto\|--dry-run]`                                                                                   | Multi-specialist PR review (security, quality, errors, perf, plus conditional typescript, react, infra, claude-md) using parallel native Claude Code subagents. Posts inline comments. Ships as a skill (`SKILL.md` format) but stays user-invoked only (`disable-model-invocation: true`) — Claude never auto-invokes it.                                                                                                                   |
| `docs`              | `/docs:audit-docs` · `/docs:enrich-claude-md`                                                                                    | Scans CLAUDE.md / READMEs / `docs/` trees / `.claude/commands` / `.claude/skills` / `.claude/rules` / architecture docs for stale claims and reports findings with suggested fixes; or investigates the codebase for useful, non-obvious facts missing from CLAUDE.md / `.claude/rules` and proposes additions. Both ship as skills, so Claude can auto-invoke them in addition to the `/` commands.                                         |
...
**Note:** all seven git workflows ship as skills (`plugins/git/skills/`), so besides the `/git:*` commands Claude can auto-invoke them when asked in natural language (e.g. "commit and push this").
...
**Invoke:** `/docs:audit-docs [file|dir|glob]` — no argument audits the whole repo; pass a file, directory, or glob to narrow the scope.
...
1. Locates every doc file matching `CLAUDE.md`, `README.md`, `.claude/commands/*.md`, `.claude/skills/*/SKILL.md`, `.claude/rules/*.md`, `**/docs/**/*.md`, and `*[Aa]rchitecture*.md` (skipping `node_modules`, `dist`, etc.).
...
**Note:** `audit-docs` and `enrich-claude-md` now ship as skills (`plugins/docs/skills/`), so besides `/docs:audit-docs` / `/docs:enrich-claude-md` Claude can auto-invoke them when it judges the docs need auditing or enriching.
...
| `pnpm test`        | `make -C plugins/code-review/tools/code-review-helper test`                                                                                                                        | Runs the `code-review` Go test suite (`go test ./...`). No JS/TS suite exists; the plugin-session-auditor session-parser ships no test suite (it runs via `go run .`).                  |
...
plugins/<name>/                                 # one directory per plugin
  ...
  commands/<file>.md                            # single slash command, <plugin>.md
  skills/<name>/SKILL.md                        # a plugin may ship skills instead (e.g. transcript, docs, git, simplify, jira, code-review)
