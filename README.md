...
A Claude Code [plugin marketplace](https://docs.claude.com/en/docs/claude-code/plugin-marketplaces) shipping one slash command and sixteen skills the author uses for everyday Git, code-review, documentation, and reasoning workflows.
...
| `debate`            | `/debate <claim>`                                                                                                            | Adversarial pro/con debate — opening arguments, then up to 5 rounds of attack/defend, then an inline markdown report of surviving, negated, and disputed arguments. Ships as a skill (`SKILL.md` format) but stays user-invoked only (`disable-model-invocation: true`) — Claude never auto-invokes it.                                                                                                                                      |
...
plugins/<name>/                                 # one directory per plugin
  ...
  skills/<name>/SKILL.md                        # a plugin may ship skills instead (e.g. transcript, docs, git, simplify, jira)
