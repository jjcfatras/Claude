...
- **Safer alternative already in the file?** If the same file (or a nearby sibling) already imports/uses the safe counterpart of the pattern you're flagging — `timingSafeEqual` beside a plain `===` credential compare, parameterized queries beside string-built SQL, `.safeParse()` beside a bare `.parse()` — the author demonstrably knew the safe pattern and applied it inconsistently: treat that as clear evidence and score high (75–90). Don't misread the in-file safe usage as "handled elsewhere" unless it actually covers the flagged input.
- **Repo convention?** Before treating a construct as a defect (a cast, a wrapper pattern, an error-handling or spec-structure shape), Grep/Read 2–3 sibling files or methods that use the same construct. If siblings use it identically, it's an established convention, not a defect — score low. Exception: a demonstrably exploitable bug or vulnerability is not excused by repetition; score that on impact.
...
3. **Scan the diff** for issues in your domain. The diff path is the bundle's `DIFF` value — don't refetch via `gh pr diff`. Use `Read` for surrounding context (function signatures, imports, call sites) when the diff alone isn't enough. **Use `Read` to confirm every `line` value before emitting a finding** — see the `line` field rule under "Findings file schema". Hunk-math drift is the single most common reason findings get demoted to summary-only.
   ...
   **Verify the claim itself, not just the line.** An "X is missing" finding requires reading the file and confirming X really is absent — never trust a grep match, the diff hunk alone, or a remembered pattern. For high-impact claims (auth bypass, injection, IDOR, race, data leak), trace the runtime path from the wired-up entry point (route registration, event handler, exported API) that actually triggers it. A finding whose triggering path you cannot construct is "plausible but unverified": score it ≤50 and cap severity at what you actually demonstrated — the gate handles the rest.

   **Read the callee before trusting a call site.** When the diff adds a call to an in-house wrapper or a configurable library API (data-access helper, queue/worker factory, client wrapper), Read the callee's implementation or docs — silent defaults (implicit result limits, fallbacks, retries, truncation) and mandatory pairings are invisible at the call site. If you can't verify, lower confidence.
...
- Emit a `suggested_fix` identical to `code`. The two render as side-by-side fenced blocks, so an identical (or near-identical) fix reads as "no change." Use `null` when there's no concrete change, and keep both fields tight so a real change is visible — see the `suggested_fix` field rule below.
...
- `line` — line **in the new version of the file**, as it would appear when checked out at HEAD_SHA. Not the position within a hunk, not the diff-line offset.
  ...
  **Don't compute this from the diff alone.** Hunk math is error-prone: multi-hunk files shift offsets, deletions invalidate naive counting, `\ No newline at end of file` markers and rename headers throw off line indices. Specialists routinely emit lines that drift by 1–10+ from the truth, which then either snap noisily or demote to summary-only. The fix is direct: confirm line numbers against the bundle's `## Source at HEAD` content, or via `git show $HEAD_SHA:<path>` for files not embedded — the working tree is not guaranteed to be at HEAD*SHA. Locate the issue in the file content at HEAD and copy that line number into `line`. The diff is for finding \_what* to flag; the file at HEAD is for confirming _where_ it lives. For multi-line issues, use the HEAD content to set both `startLine` and `line`.
...
- `suggested_fix` — **required when the finding has a concrete code-level fix** — i.e., a self-contained replacement that goes on the cited line(s). Set to `null` only when no single-snippet replacement applies: architectural findings (rename a module, restructure a folder), findings that span many non-contiguous sites, or findings where picking between several valid fixes needs human judgment. Single-block code change → string. Architectural/conceptual finding → `null`. The renderer wraps the value in a fenced block tagged with `language`, so write only code (or a minimal patch excerpt with surrounding context if it wouldn't read standalone) — no backticks, headings, or prose prefixes like "Fix:". Put reasoning in `explanation`. **Scope tightly:** keep both `code` and `suggested_fix` to the line(s) you're changing plus the minimum context needed to read standalone — don't reproduce a large unchanged block when the fix touches one line or token. `code` and `suggested_fix` render as two side-by-side fenced blocks, so a one-token change buried in a multi-line copy reads as "no change." **Must differ:** if your replacement would be byte-identical to the cited `code`, there is no concrete fix — set `suggested_fix` to `null`. (The helper drops a fix equal to the code as a backstop, but don't rely on it.)
...
- General code-quality concerns (test coverage, documentation) unless explicitly required in CLAUDE.md. This covers _missing_ tests and docs — defects in tests the diff itself adds or modifies, and docs the PR itself modifies that cite symbols absent from the codebase, are in scope.
