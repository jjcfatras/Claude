...
Investigate the codebase for useful, durable, non-obvious facts that would help a future Claude session but aren't yet captured in the project's memory files (`CLAUDE.md`, `.claude/rules/*.md`). This is the inverse of `/audit-docs`: that command finds claims in the docs that are now _wrong_; this one finds facts in the _code_ that are missing from the docs and worth adding.
...
A proposed addition whose best home is a `.claude/rules/` file that doesn't exist yet is a special case: only create the file (via `Write`) if the user approves that addition, and confirm the filename with them first. Batching the _questions_ is encouraged; do **not** batch-_apply_ — each addition keeps its own decision so the human checkpoint between proposal and mutation stays intact.
