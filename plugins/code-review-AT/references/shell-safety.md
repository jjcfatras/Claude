...
9. **Don't `ls` a shared work directory to detect peer completion.** Every `ls /tmp/.../findings/` to "see what specialists have written" is a polling re-implementation of state that DM signaling already carries. Use `SendMessage`'s `scan_complete`-style DM (or the peer's `TaskOutput`) for completion; reach for the Glob tool only for the rare one-shot directory listing where you actually need a file count rather than a peer signal. The lead in `code-review` is bound to this rule by step 2e of the command file — wake-turn enumeration must use `Glob`, never `Bash ls` or `Bash find`. Real failure observed in transcript `b5a8dd9d` (May 2026): lead used `Bash ls` 3× on wake-turns. The contract held only because `Bash(ls:*)` was allowlisted; on a more locked-down project it would have wedged.
