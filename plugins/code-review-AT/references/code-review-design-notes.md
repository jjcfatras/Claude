...
Maintainer-only context for `${CLAUDE_PLUGIN_ROOT}/commands/code-review-AT.md` and `${CLAUDE_PLUGIN_ROOT}/agents/code-review-*.md`. Not read by the skill at runtime. Capture rationale here so the runtime prompts stay terse.
