...
8. **No `sleep N` as a scan-completion, grace, or teardown timer.** Use `Monitor` with an until-loop, or rely on the DM-driven `SendMessage` signal a peer is already required to send. The harness gives you a real wake on event, not a fixed wall clock; polling sleeps both prompt under default mode and burn the prompt cache in auto mode. Backgrounded `sleep` (with `run_in_background: true`) is acceptable only when you genuinely want a wall-clock delay (e.g., a rate-limit cooldown), not when you're polling for state. The Bash tool description ("Use the Monitor tool to stream events from a background process") is the same rule from the harness side.

9. **Don't `ls` a shared work directory to detect peer completion.** Every `ls /tmp/.../findings/` to "see what specialists have written" is a polling re-implementation of state that DM signaling already carries. Use `SendMessage`'s `scan_complete`-style DM (or the peer's `TaskOutput`) for completion; reach for the Glob tool only for the rare one-shot directory listing where you actually need a file count rather than a peer signal.

10. **Use the Glob tool for "newest file" lookups.** `ls -lt … | head -1` (or any `ls | sort | head` pipeline) prompts on every component, eats an `allowed-tools` slot for `ls`, and is fragile across macOS/Linux. The Glob tool returns matches sorted by mtime by default; use `pattern: "…/*.jsonl"` and read the first result.
